...
mod tests {
    ...
    fn parse_bundled_default_matches_worker_schema() {
        ...
        // 2026-08-24 Sprint 18: 16→17 categories (ミックス 7 追加)、96→102 preset
        ...
        assert_eq!(
            ...
            17,
            "seed data: 17 categories (Both auth + FieldTest + 生活雑貨 + 趣味 DIY + 工具ホルダー + 電子機器ケース + バスルーム/ガレージ + キッチン + 3Dプリンタ周辺 + 引き出し/壁 + ミックス + ミックス 2 + ミックス 3 + ミックス 4 + ミックス 5 + ミックス 6 + ミックス 7)"
        );
        let total_presets: usize = parsed.categories.iter().map(|c| c.presets.len()).sum();
        assert_eq!(total_presets, 102, "seed data: 102 preset total");
    }
