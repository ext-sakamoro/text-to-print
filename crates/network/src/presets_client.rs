...
mod tests {
    ...
    fn parse_bundled_default_matches_worker_schema() {
        ...
        // 2026-08-23 Sprint 6: 4→5 categories (工具ホルダー 追加)、20→28 preset
        ...
        assert_eq!(
            ...
            5,
            "seed data: 5 categories (Both auth + FieldTest + 生活雑貨 + 趣味 DIY + 工具ホルダー)"
        );
        let total_presets: usize = parsed.categories.iter().map(|c| c.presets.len()).sum();
        assert_eq!(total_presets, 28, "seed data: 28 preset total");
    }
