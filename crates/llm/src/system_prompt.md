...
- 3-param: `gridfinity_bin(ux, uy, hu)` / `sticky_note_holder(pw, pd, h)` / `business_card_holder(cw, ch, st)` / `phone_stand(sw, bh, chd)` / `headphone_holder(al, hw, mw)` / `under_desk_mount(dt, cw, sd)` / `desk_shelf(sw, sd, lh)` / `monitor_riser(w, d, h)` / `tissue_box_cover(il, iw, ih)` / `storage_box(il, iw, ih)` / `skadis_panel(size, t, r)` / `card_tray(cw, ch, dep)` / `token_well(dia, d, n)` / `wrench_holder(min, max, n)` / `socket_rail(pd, ph, n)` / `hex_bit_holder(r, c, sp)` / `raspi_case(w, d, h)` / `esp32_enclosure(w, d, h)` / `battery_18650_holder(n, w, f)` / `toothbrush_holder(n, dia, h)` / `drill_bit_holder(min, max, n)` / `pliers_rack(n, w, d)`
